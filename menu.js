    ...
    const intervalId = setInterval(async () => {
      ...
      const acquireUrlData = async (urlDatum) => {
        ...
        switch (Object.keys(urlData)[0]) {
          ...
          case 'anilist':
            ...
            // eslint-disable-next-line no-undef
            urlData.kitsu = urlData.mal ? await matchOnKitsuFromMAL(urlData.mal) : false;
            return urlData;
          case 'kitsu':
            urlData.mal = await matchOnMALFromKitsu(urlData.kitsu); // eslint-disable-line no-undef
            // eslint-disable-next-line no-undef
            urlData.anilist = urlData.mal ? await matchOnAniListFromMAL(urlData.mal) : false;
            return urlData;
          default:
            return false;
        }
      ...
      browser.contextMenus.onClicked.addListener(async (info, tab) => {
        ...
        if (urlData) {
          ...
          createNotification({ title: 'PTW extending results', message });
        } else {
