...
const matchOnKitsuFromMAL = async (urlData) => {
  ...
  let errors = null;
  try {
    [data, meta, errors] = await sendKitsuQuery(
      null,
      'GET',
...
const matchOnMALFromKitsu = async (urlData) => {
  ...
  try {
    ...
    return {
      type: mal.attributes.externalSite.substring(12),
      id: mal.attributes.externalId,
    };
  } catch (e) {
    console.log('Unsuccessfully made request', e);
    return false;
...
const handleKitsu = async (tab, urlData, options) => {
  ...
  try {
    ...
    if (meta.count !== 0) {
      [data, meta, errors] = await sendKitsuQuery(options.accessToken, 'PATCH', `library-entries/${entryId}`, {
        data: {
          type: entryType,
          id: entryId,
          attributes: {
      ...
      [data, meta, errors] = await sendKitsuQuery(options.accessToken, 'POST', 'library-entries/', {
        data: {
          type: entryType,
          attributes: {
            status: 'planned',
