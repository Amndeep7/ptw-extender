...
const saveOption = async (option, modifiers) => {
  ...
  try {
    ...
    options[optionsVersion][modifiers.type][option.target.name] = modifiers.isProperty
      ? option.target[modifiers.property]
      : modifiers.property;
    ...
    if (modifiers.isProperty) {
      document.querySelector('#results').innerHTML = `${option.target.name} was successfully saved to ${
        option.target[modifiers.property]
      }`;
    } else {
      document.querySelector(
        '#results'
      ).innerHTML = `${option.target.name} was successfully saved regarding ${option.target.value}`;
    }
  } catch (e) {
...
const setupSavingCheckboxOptions = () => {
  document.querySelectorAll('input[type=checkbox]:not([value])').forEach((option) =>
...
const setupSavingMultipleCheckboxOptions = () => {
  document.querySelectorAll('fieldset[name]').forEach((fieldset) =>
    fieldset.addEventListener('change', (o) => {
      const values = {};
...
const setupSavingTextareaOptions = () => {
  document.querySelectorAll('textarea').forEach((option) =>
    option.addEventListener('input', (o) => {
      saveOption(o, {
        type: 'textarea',
        isProperty: true,
        property: 'value',
...
const setupAniListCustomLists = async (accessToken) => {
  ...
  if (errors) {
    ...
    document.querySelector('#results').innerHTML = 'Failed to retrieve AniList custom lists - authentication failed';
  }
  ...
  const reducer = (customLists, list) => {
    customLists[list] = false;
    return customLists;
  };
  // eslint-disable-next-line no-undef
...
const restoreLoginButton = (site, options) => {
  ...
  try {
    document.querySelector(`input[name='${site.raw}_login']`).value =
      options.authentication[site.raw].accessToken === null ? `Login to ${site.proper}` : `Logout of ${site.proper}`;
  ...
  document.querySelector("input[name='anilist_login']").addEventListener('click', async (event) => {
    try {
      ...
      if (options[optionsVersion].authentication.anilist.accessToken === null) {
        ...
        const clientId =
          // eslint-disable-next-line no-undef
          extensionConstants[extensionConstantsVersion].authentication.anilist[
            // eslint-disable-next-line no-undef
            `clientId_${extensionConstants[extensionConstantsVersion].browser.type}`
          ];
        const redirectRequestUrl = `https://anilist.co/api/v2/oauth/authorize?response_type=token&client_id=${clientId}`;
        ...
        try {
          ...
          if (accessToken) {
            ...
            document.querySelector('#results').innerHTML =
              "Couldn't log into Kitsu probably due to incorrect credentials";
          }
        } catch (e2) {
          event.target.value = 'Login to Kitsu';
...
const setupDisablingDependentOptions = () => {
  ...
  document.querySelectorAll('.dependent').forEach((dependent) => {
    ...
    dependent.querySelectorAll('input, textarea').forEach((input) => {
      ...
      const update = (o) => {
        input.dependencies[dependent.dataset.dependentOn] = o[dependent.dataset.dependentProperty] === value;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const intervalId = setInterval(async () => {
    ...
    const reviewLoc =
      // eslint-disable-next-line no-undef
      extensionConstants[extensionConstantsVersion].browser.type === 'firefox'
